...
DEMISTO_ERROR_LOGGER: ErrorHandler = ErrorHandler()
...
def get_edl_ioc_values(on_demand: bool,
    ...
    if on_demand:
        if request_args.is_request_change(edl_cache):
            values_str = get_ioc_values_str_from_context(edl_cache, request_args=request_args, iocs=current_iocs)

        else:
            values_str = get_ioc_values_str_from_context(edl_cache, request_args=request_args)
    else:
        if last_run:
...
def get_ioc_values_str_from_context(edl_cache: dict,
                                    request_args: RequestArguments,
                                    iocs: list = None) -> str:
    """
    ...
    if iocs:
        ...
        demisto.setIntegrationContext(edl_cache)
...
def run_long_running(params, is_test=False):
    ...
    try:
        ...
        server = WSGIServer(('0.0.0.0', port), APP, **ssl_args, log=DEMISTO_LOGGER, error_log=DEMISTO_ERROR_LOGGER)
        if is_test:
